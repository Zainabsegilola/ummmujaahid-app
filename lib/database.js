...
export const addCardToDeckWithTranslation = async (deckId, arabicWord, context, videoTimestamp, userId, translationData = null) => {
  try {
    const cardData = {
      deck_id: deckId,
      user_id: userId,
...
// Add this ORIGINAL function back to your database.js
// Updated addCardToDeck to save translation data
...
// Also add the original addQuranCard function
export async function addQuranCard(deckId, arabicWord, surahNumber, verseNumber, wordPosition, context, userId, translationData = null) {
  try {
    ...
    console.log('Saving Quran card with data:', cardData);
...
export async function restoreDeletedDeck(deletedRecordId, userId) {
  ...
  // ADD these functions to the bottom of your database.js file
// (Replace the previous ones if you already added them)
...
export async function saveTranscriptToCache(videoId, transcript, lang) {
  try {
    console.log('💾 Saving transcript to cache:', {
      videoId,
      lang,
    ...
    console.error('❌ Error in saveTranscriptToCache:', error);
    return { data: null, error };
  }
}

// Check if transcript exists (lightweight check)
export async function transcriptExists(videoId) {
  try {
    const { data, error } = await supabase
      .from('transcripts')
      .select('video_id')
